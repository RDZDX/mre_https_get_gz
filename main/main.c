...
//    if (flightMode == VM_TRUE) {return;}

    create_selfapp_txt_filename(file_pathw, (char*)"txt");
...
void read_file_to_input(VMWSTR path)
{
    VMCHAR ascii_data[1200];
    VMCHAR ucs2_data[1200];
    VMUINT nread;
    VMFILE f_read;

    f_read = vm_file_open(path, MODE_READ, FALSE);
    vm_file_read(f_read, ascii_data, sizeof(ascii_data) - 1, &nread);
    ascii_data[nread] = '\0';
    vm_file_close(f_read);
    trim_right(ascii_data);
    vm_ascii_to_ucs2((VMWSTR)ucs2_data, (strlen(ascii_data) + 1) * 2, ascii_data);
    vm_input_text3((VMWSTR)ucs2_data, 1200, 8, save_text);
