...
//static int last_tcp_event = -1;
//static volatile int can_write = 0;
...
void vm_main(void)
{
    ...
    vm_create_timer(1000, timer);

//    dbg_timer = vm_create_timer(1000, dbg_timer_cb);
...
//vm_ascii_to_ucs2(wname, sizeof(wname), "e:\\https_download.bin");
//g_file = vm_file_open(wname, MODE_CREATE_ALWAYS_WRITE, TRUE);

//if (g_file < 0)
//{
////    display_text_line(screenbuf, "Cannot create file", 0, filledDsplByLines, nscreen_width, nscreen_height, 2, 1,
...
//        display_text_line(screenbuf, "before ssl write", 0, filledDsplByLines, nscreen_width, nscreen_height, 2, 1,
//            VM_COLOR_BLACK, VM_FALSE);
...
//    {
//        display_text_line(screenbuf, "SSL write failed", 0, filledDsplByLines, nscreen_width, nscreen_height, 2, 1,
//            VM_COLOR_BLACK, VM_FALSE);


//        int err = br_ssl_engine_last_error(&https_ctx.sc.eng);
//        char msgxx[64];
//        sprintf(msgxx, "SSL err=%d", err);
//        display_text_line(screenbuf, msgxx, 0, filledDsplByLines, nscreen_width, nscreen_height, 2, 1,
//            VM_COLOR_BLACK, VM_FALSE);
...
//        (unsigned long)br_ssl_engine_current_state(
//            &https_ctx.sc.eng));
...
//   display_text_line(screenbuf, "after ssl write", 0, filledDsplByLines, nscreen_width, nscreen_height, 2, 1,
//   VM_COLOR_BLACK, VM_FALSE);
...
//        display_text_line(screenbuf, text, 0, filledDsplByLines, nscreen_width, nscreen_height, 2, 1,
//        VM_COLOR_BLACK, VM_FALSE);
        log_text(text);

//        display_text_line(screenbuf, "SSL flush failed", 0, filledDsplByLines, nscreen_width, nscreen_height, 2, 1,
//        VM_COLOR_BLACK, VM_FALSE);
//        log_text("SSL flush failed");
...
//            display_text_line(screenbuf, msg, 0, filledDsplByLines, nscreen_width, nscreen_height, 2, 1,
//            VM_COLOR_BLACK, VM_FALSE);
            log_text(msg);
...
//    https_ctx.tcp_handle = vm_tcp_connect(url->host, 443, VM_TCP_APN_CMNET, tcp_callback);
//    https_ctx.tcp_handle = vm_tcp_connect(url->host, 443, 1, tcp_callback);
//    https_ctx.tcp_handle = vm_tcp_connect("93.184.216.34", 80, 1, tcp_callback);
...
if (!strncmp(redirect_url, "https://", 8))
{
    ...
    else
    {
        log_text("bad redirect url");
             ...
             7))
{
    log_text("http redirect not supported");
}
