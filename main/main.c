...
//static br_x509_knownkey_context noverify_x509;
...
//memset(&noverify_x509, 0, sizeof(noverify_x509));

//noverify_x509.vtable = &br_x509_knownkey_vtable;

//br_ssl_engine_set_x509(&https_ctx.sc.eng, &noverify_x509.vtable);
...
//vm_get_time(&t); //disable if no certificate check
//unsigned long days = bearssl_days(t.year, t.mon, t.day);
//unsigned long seconds = t.hour * 3600UL + t.min  * 60UL + t.sec;
//br_x509_minimal_set_time(&https_ctx.xc, days, seconds); //disable if no certificate check
...
void log_text(const char *msg)
{
    ...
    display_text_line(screenbuf, msg, 0, filledDsplByLines, nscreen_width, nscreen_height, 2, 1, VM_COLOR_BLACK, VM_FALSE);
}
...
static void insecure_start_chain(
    const br_x509_class **ctx,
    const char *server_name)
{

//log_text("start_chain"); 

   br_x509_insecure_context *xc =
        (br_x509_insecure_context *)ctx;
...
static void insecure_start_cert(
    const br_x509_class **ctx,
    uint32_t length)
{
...
static unsigned insecure_end_chain(
    const br_x509_class **ctx)
{
    br_x509_insecure_context *xc =
        (br_x509_insecure_context *)ctx;
...
static const br_x509_pkey *insecure_get_pkey(
    ...
    unsigned *usages)
{
    const br_x509_insecure_context *xc =
        (const br_x509_insecure_context *)ctx;
    ...
    return br_x509_decoder_get_pkey(
        (br_x509_decoder_context *)&xc->ctx);
}

void br_x509_insecure_init(
    br_x509_insecure_context *ctx)
{
