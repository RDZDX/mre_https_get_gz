...
void vm_main(void)
{
    ...
    vm_ascii_to_ucs2(wname, sizeof(wname), "e:\\download.gz");
    vm_file_delete(wname);
    vm_ascii_to_ucs2(wname, sizeof(wname), "e:\\download.html");
    vm_file_delete(wname);
...
static VMINT download_process(const VMCHAR* url,
    ...
    if (strncmp(url, "http://", strlen("http://"))) { //strlen("http:\/\/") ????????
...
//vm_file_rename(old_name, new_name);
